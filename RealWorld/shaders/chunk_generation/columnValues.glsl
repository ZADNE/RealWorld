...
float linColumnValue_x(float x, float columnSize, float seed){
	vec3 vals = columnValues(x, columnSize, seed);
	return mix(vals.x, vals.y, linStep_x(vals.z));
}

float smoothColumnValue_x(float x, float columnSize, float seed){
	vec3 vals = columnValues(x, columnSize, seed);
	return mix(vals.x, vals.y, smoothStep_x(vals.z));
}

float smootherColumnValue_x(float x, float columnSize, float seed){
	vec3 vals = columnValues(x, columnSize, seed);
...
vec2 linColumnValue_x_dx(float x, float columnSize, float seed){
	vec3 vals = columnValues(x, columnSize, seed);
	vec2 step = linStep_x_dx(vals.z);
	return vec2(mix(vals.x, vals.y, step.x), step.y * (vals.y - vals.x));
}

vec2 smoothColumnValue_x_dx(float x, float columnSize, float seed){
	vec3 vals = columnValues(x, columnSize, seed);
	vec2 step = smoothStep_x_dx(vals.z);
	return vec2(mix(vals.x, vals.y, step.x), step.y * (vals.y - vals.x));
}

vec2 smootherColumnValue_x_dx(float x, float columnSize, float seed){
	vec3 vals = columnValues(x, columnSize, seed);
